                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'var(--border-subtle)', borderBottom: '1px solid var(--border-default)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            {['all', 'core', 'explore'].map(f => (
                                <button key={f} className="btn btn-ghost mono" style={{ fontSize: '9px', padding: '2px 8px', border: view === f ? '1px solid var(--accent-primary)' : '1px solid transparent', color: view === f ? 'var(--accent-primary)' : 'var(--text-tertiary)' }} onClick={() => setView(f)}>
                            ...
                            {filtered.map((bet, idx) => (
                                <div key={bet.id} style={{ padding: '16px', borderBottom: idx < filtered.length - 1 ? '1px solid var(--border-subtle)' : 'none', background: idx % 2 === 0 ? 'transparent' : '#000', borderLeft: `3px solid ${statusColor(bet.status)}` }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div>
                                            <div style={{ fontWeight: 'bold', color: 'var(--text-primary)', fontFamily: 'var(--font-mono)', fontSize: '12px' }}>{bet.name.toUpperCase()}</div>
                                            <div style={{ color: 'var(--text-secondary)', fontSize: '10px', marginTop: '6px', fontFamily: 'var(--font-mono)' }}>HYP: {bet.hypothesis.toUpperCase()}</div>
                                        </div>
                                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                            ...
                                            {bet.resources && <span style={{ fontSize: '9px', padding: '4px 8px', border: '1px solid var(--text-tertiary)', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{bet.resources.toUpperCase()}</span>}
