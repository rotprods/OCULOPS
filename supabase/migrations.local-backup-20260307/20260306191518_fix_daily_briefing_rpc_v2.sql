...
BEGIN
  ...
  LIMIT 1;
  IF v_pipeline_yesterday IS NULL THEN v_pipeline_yesterday := v_pipeline_total; END IF;
  ...
  LIMIT 1;
  IF v_health_score IS NULL THEN v_health_score := 85; END IF;

  SELECT COUNT(*) INTO v_tasks_today_count
  FROM tasks WHERE status != 'done' AND (due_date IS NULL OR due_date <= CURRENT_DATE);
  ...
  INTO v_stale_deals
  FROM deals d
  WHERE d.stage NOT IN ('won', 'lost') AND d.updated_at < NOW() - INTERVAL '3 days';

  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'title', s.title, 'category', s.category, 'impact', s.impact, 'source', s.source
  )), '[]'::jsonb)
  INTO v_signals
  ...
  FROM agent_registry ar WHERE ar.status != 'online';
  ...
  result := jsonb_build_object(
    ...
    'pipeline_change_pct', CASE WHEN v_pipeline_yesterday > 0
